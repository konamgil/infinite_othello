  ...
  private reportError = async (error: Error, errorInfo: React.ErrorInfo) => {
    try {
      ...
      const errorReport = {
        ...
        userId: 'user_id_here', // Would come from auth context
...
추가 정보:
  ...
  private getErrorCategory = (error: Error | null): {
    category: string;
          ...
          <div className="w-full max-w-2xl">
            ...
            <div className="bg-black/40 backdrop-blur-md border border-white/20 rounded-2xl p-6 sm:p-8">
                  ...
                  <div className="bg-black/30 rounded-lg p-4 border border-white/10">
                    ...
                    {process.env.NODE_ENV === 'development' && this.state.error.stack && (
                      <details className="mt-3">
                        ...
                        <pre className="text-xs text-white/40 mt-2 overflow-x-auto whitespace-pre-wrap">
              ...
              {this.state.retryCount > 0 && (
                <div className="mb-6 p-3 rounded-lg bg-yellow-400/10 border border-yellow-400/20">
                  <p className="text-sm text-yellow-200 font-display">
                    재시도 {this.state.retryCount}/{this.maxRetries}회 시도됨
                  </p>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                {canRetry && (
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white font-semibold rounded-lg transition-all font-display"
                  >
                    <RefreshCw size={18} />
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-white/10 hover:bg-white/20 text-white/90 font-semibold rounded-lg border border-white/20 transition-all font-display"
                >
                  <ArrowLeft size={18} />
                  이전 페이지
                </button>
              ...
              <div className="mt-6 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="text-xs text-white/50 font-display text-center sm:text-left">
                  문제가 지속되면 브라우저를 새로고침하거나 캐시를 지워보세요
                </div>

                {this.props.enableReporting && (
                  <button
                    onClick={this.handleReportBug}
                    className="flex items-center gap-2 px-4 py-2 text-sm text-white/60 hover:text-white/80 transition-colors font-display"
                  >
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-white/40 font-display">
                이 오류는 자동으로 기록되며, 개발팀이 빠른 시일 내에 해결하겠습니다
              </p>
            </div>
          </div>
        </div>
...
// Functional wrapper component for hooks-based usage
...
export const SimpleErrorFallback: React.FC<{ error?: Error; retry?: () => void }> = ({
  ...
  <div className="flex flex-col items-center justify-center p-8 text-center">
    ...
    <p className="text-white/60 text-sm mb-4 font-display">
      {error?.message || '알 수 없는 오류가 발생했습니다'}
    </p>
    {retry && (
      <button
        onClick={retry}
        className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors font-display"
      >
        다시 시도
      </button>
    )}
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-xs text-white/60 font-display">
          {error?.message || '리플레이 데이터를 처리할 수 없습니다'}
        </p>
      </div>
    </div>
    {retry && (
